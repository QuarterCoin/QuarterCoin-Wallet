...
The RPM spec file provided here is for Quartercoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://quartercoin.co/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Quartercoin git repository](https://github.com/QuarterCoin/Quarter-Coin-Wallet) but are not part of
...
Currently only `Source10` is used, for the example `quartercoin.conf` file.
...
in the quartercoin git repository. At present only an SVG version of the Quartercoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the quartercoin
...
needed when 0.12.1 is released, a proper fix is already in the Quartercoin git
master branch.
...
idea to build Quartercoin Core against a static version of that library compiled
...
The `quartercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `quartercoin-server` package. At this point, most RPM based
...
The `quartercoin-server` package should have SELinux support. How to properly do
...
Quartercoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/quartercoin/). He has been
packaging Quartercoin for Fedora at least since 2012.
...
4. I renamed the `quartercoin` package that contains the Qt GUI to `quartercoin-core` as
that appears to be how the general population refers to it, in contrast to
`quartercoin-xt` or `quartercoin-classic`. I wanted to make sure the general population
