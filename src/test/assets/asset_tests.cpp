...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of QTCCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("QTC", type));
        BOOST_CHECK(!IsAssetNameValid("QTC", type));
        BOOST_CHECK(!IsAssetNameValid("QTCCOIN", type));

        //- Versions of QTCCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("QTC.COIN", type));
        BOOST_CHECK(IsAssetNameValid("QTC_COIN", type));
        BOOST_CHECK(IsAssetNameValid("QTCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERQTC", type));
        BOOST_CHECK(IsAssetNameValid("QTCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEQTC", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_QTCS", type));
        BOOST_CHECK(IsAssetNameValid("SEQTCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("QTC", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
