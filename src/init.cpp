 ...
 * between quartercoind, and quartercoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("quartercoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateQuartercoins(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), QTC_CONF_FILENAME));
    if (mode == HMM_QTCD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), QTC_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/QuarterCoin/Quarter-Coin-Wallet>";
    const std::string URL_WEBSITE = "<https://quartercoin.co>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("quartercoin-loadblk");
...
/** Sanity checks
 *  Ensure that Quartercoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Quartercoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Quartercoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", QTC_CONF_FILENAME)).string());
    ...
    GenerateQuartercoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
