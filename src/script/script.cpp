...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** QTC START */
    case OP_QTC_ASSET              : return "OP_QTC_ASSET";
    /** QTC END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_QTC_ASSET) { // OP_QTC_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == QTC_R) { // Check to see if QTC starts at 27 ( this->size() < 105)
                if ((*this)[28] == QTC_V)
                    if ((*this)[29] == QTC_N)
                        index = 30;
            } else {
                if ((*this)[28] == QTC_R) // Check to see if QTC starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == QTC_V)
                        if ((*this)[30] == QTC_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == QTC_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == QTC_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == QTC_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == QTC_R) {
