...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    if (nHeight <= DEV_FUND_UNTIL) {
      ...
      coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(CQuartercoinAddress(address).Get());
...
void static QuartercoinMiner(const CChainParams& chainparams)
{
    LogPrintf("QuartercoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quartercoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("QuartercoinMiner -- Wallet not available\n");
    }
        ...
        while (true) {
            ...
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("QuartercoinMiner -- waiting for connections and blocks downloaded\n");
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("QuartercoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("QuartercoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("QuartercoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("QuartercoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateQuartercoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&QuartercoinMiner, boost::cref(chainparams)));
    }
