...
#include "config/quartercoin-config.h"
#endif

#include "quartercoingui.h"

#include "quartercoinunits.h"
...
const std::string QuartercoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString QuartercoinGUI::DEFAULT_WALLET = "~Default";

QuartercoinGUI::QuartercoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void QuartercoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Quartercoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and quartercoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to QTC addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Quartercoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Quartercoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a quartercoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Quartercoin command-line options").arg(tr(PACKAGE_NAME)));
...
void QuartercoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void QuartercoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** QTC START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/quartercointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* quartercoinLabelLayout = new QVBoxLayout(toolbarWidget);
        quartercoinLabelLayout->addWidget(label);
        quartercoinLabelLayout->addWidget(toolbar);
        quartercoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        quartercoinLabelLayout->addStretch(1);
...
void QuartercoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool QuartercoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool QuartercoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void QuartercoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QuartercoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void QuartercoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void QuartercoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void QuartercoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void QuartercoinGUI::optionsClicked()
{
...
void QuartercoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QuartercoinGUI::showDebugWindow()
{
...
void QuartercoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void QuartercoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void QuartercoinGUI::openClicked()
{
...
void QuartercoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QuartercoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QuartercoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QuartercoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QuartercoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QuartercoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** QTC START */
void QuartercoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void QuartercoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void QuartercoinGUI::gotoManageAssetsPage()
{
...
void QuartercoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Quartercoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void QuartercoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QuartercoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void QuartercoinGUI::updateHeadersSyncProgressLabel()
{
...
void QuartercoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void QuartercoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Quartercoin"); // default title
...
void QuartercoinGUI::changeEvent(QEvent *e)
{
...
void QuartercoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QuartercoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QuartercoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "QTC")
        msg += tr("Amount: %1\n").arg(QuartercoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(QuartercoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void QuartercoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QuartercoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QuartercoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QuartercoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QuartercoinGUI::setHDStatus(int hdEnabled)
{
...
void QuartercoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QuartercoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QuartercoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QuartercoinGUI::detectShutdown()
{
...
void QuartercoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void QuartercoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void QuartercoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QuartercoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void QuartercoinGUI::subscribeToCoreSignals()
{
...
void QuartercoinGUI::unsubscribeFromCoreSignals()
{
...
void QuartercoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<QuartercoinUnits::Unit> units = QuartercoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QuartercoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(QuartercoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (QuartercoinUnits::Unit u : QuartercoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(QuartercoinUnits::name(u)), this);
...
{
    setText(QuartercoinUnits::name(newUnits));
}
...
void QuartercoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=QTCBTC"));
