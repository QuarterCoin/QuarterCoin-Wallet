...
`test/functional/test_framework/test_framework.py:QuartercoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/quartercoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_quartercoind = config["components"].getboolean("ENABLE_QTCD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/QuarterCoin/QuaterCoin-Wallet/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/QuarterCoin/QuaterCoin-Wallet/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_quartercoind):
        print("No functional tests to run. Wallet, utils, and quartercoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if quartercoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "quartercoind"]) is not None:
            print("%sWARNING!%s There is already a quartercoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "QTCD" not in os.environ:
        os.environ["QTCD"] = build_dir + '/src/quartercoind' + exeext
        os.environ["QTCCLI"] = build_dir + '/src/quartercoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `quartercoin-cli help` (`rpc_interface.txt`).
